...
const applyFilters = (tasks, query) => tasks
  .filter((task) => {
    let matches = true;
...
const applyPagination = (tasks, page, limit) => tasks
...
const TaskListTable = (props) => {
  ...
  const filteredTasks = applyFilters(tasks, query);
  const sortedTasks = applySort(filteredTasks, sort);
  const paginatedTasks = applyPagination(sortedTasks, page, limit);
              ...
              {paginatedTasks.map((task) => {
                return (
                  ...
                  >
                    ...
                    <TableCell>
                      {task.updateDate ? (moment(task.updateDate).format("dddd, MMM DD HH:mm a")) : (<p>No Update</p>)}
                    </TableCell>
                    <TableCell>
                      {task.status == "CHECKED" ? (
